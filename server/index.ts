...
app.get('/api/overview', async (_req, res) => {
  ...
  // 代码安全审计相关数据
  const recentCodeAudits = await db.all<
    Array<{
      id: number
      name: string
      status: string
  ...
  >(
    `SELECT id, name, status, findings_count, risk_score, created_at as createdAt
     FROM code_audits
     ORDER BY created_at DESC
     LIMIT 5;`,
  )

  const codeAuditTrend = await db.all<Array<{ day: string; cnt: number }>>(
    `
      SELECT
        strftime('%Y-%m-%d', datetime(created_at / 1000, 'unixepoch')) as day,
        COUNT(1) as cnt
  ...
  const severityDistribution = (await db.all(
    `SELECT severity, COUNT(1) as cnt
     FROM code_audit_items
     WHERE status = 'confirmed'
     GROUP BY severity;`,
  )) as Array<{ severity: string; cnt: number }>

  const cweDistribution = (await db.all(
    `SELECT cwe_id as cweId, cwe_name as cweName, COUNT(1) as cnt
     FROM code_audit_items
     WHERE status = 'confirmed' AND cwe_id IS NOT NULL
     GROUP BY cwe_id
     ORDER BY cnt DESC
     LIMIT 8;`,
  )) as Array<{ cweId: string; cweName: string; cnt: number }>

  // MCP 严重度分布（从 mcp_reports.findings JSON 解析）
  const mcpReports = await db.all<Array<{ findings: string }>>(
    `SELECT findings FROM mcp_reports WHERE findings IS NOT NULL;`
  )
  const mcpSeverityDistribution: Array<{ severity: string; cnt: number }> = []
  const mcpSevCount: Record<string, number> = { critical: 0, high: 0, medium: 0, low: 0, info: 0 }
    ...
    try {
      ...
      if (Array.isArray(findings)) {
        for (const f of findings) {
          ...
          if (mcpSevCount[sev] !== undefined) mcpSevCount[sev]++
        }
      }
    } catch { /* skip */ }
  }
  for (const [sev, cnt] of Object.entries(mcpSevCount)) {
    if (cnt > 0) mcpSeverityDistribution.push({ severity: sev, cnt })
  }

  // Skills 安全审计相关数据
  const recentSkillsAudits = await db.all<
    Array<{
      id: number
      name: string
      status: string
  ...
  >(
    `SELECT id, name, status, findings_count, risk_score, risk_level, created_at as createdAt
     FROM skills_audits
     ORDER BY created_at DESC
     LIMIT 5;`,
  )

  const skillsAuditTrend = await db.all<Array<{ day: string; cnt: number }>>(
    `
      SELECT
        strftime('%Y-%m-%d', datetime(created_at / 1000, 'unixepoch')) as day,
        COUNT(1) as cnt
  ...
  const skillsSeverityDistribution = (await db.all(
    `SELECT severity, COUNT(1) as cnt
  ...
  const skillsRiskCategoryDistribution = (await db.all(
    `SELECT risk_category, COUNT(1) as cnt
     FROM skills_audit_items
     WHERE status != 'false_positive'
     GROUP BY risk_category
     ORDER BY cnt DESC
     LIMIT 10;`,
  )) as Array<{ riskCategory: string; cnt: number }>
...
const MCP_EVAL_DEFAULTS = {
  ...
  model: '',
  systemPrompt:
    '你是 MCP Server 的资深安全审计专家。以自动化扫描 Agent 的方式工作：1) 信息收集 2) 代码安全审计 3) 漏洞整理 4) 可利用性复核 5) 输出报告。结论必须基于证据，避免臆测。',
  timeoutMs: 120000,
}
...
app.get('/api/prompt-collections', async (req, res) => {
  ...
  const collections = await db.all<Array<{
    id: string; name: string; library: string; description: string | null;
    prompt_count: number; created_at: number; updated_at: number
  }>>(
    `SELECT id, name, library, description, prompt_count, created_at, updated_at FROM prompt_collections WHERE ${where} ORDER BY created_at DESC;`,
    params,
  )
...
app.post('/api/prompt-collections', async (req, res) => {
  ...
  const now = Date.now()
  await db.run(
    'INSERT INTO prompt_collections (id, name, library, description, prompt_count, created_at, updated_at) VALUES (?, ?, ?, ?, 0, ?, ?);',
    [id, parsed.data.name, parsed.data.library, parsed.data.description ?? null, now, now],
  )
  res.json({ id })
})
...
app.get('/api/prompts', async (req, res) => {
  ...
  const countRow = await db.get<{ total: number }>(`SELECT COUNT(*) as total FROM prompts p WHERE ${where};`, params)
  const total = countRow?.total ?? 0
  const offset = (page - 1) * pageSize
  const items = await db.all<
    Array<{ id: string; library: string; riskType: string; riskSubType: string | null; prompt: string; createdAt: number; collectionId: string | null; collectionName: string | null }>
  >(`SELECT p.id, p.library, p.riskType, p.riskSubType, p.prompt, p.createdAt, p.collection_id as collectionId, pc.name as collectionName FROM prompts p LEFT JOIN prompt_collections pc ON p.collection_id = pc.id WHERE ${where} ORDER BY p.createdAt DESC LIMIT ? OFFSET ?;`, [...params, pageSize, offset])
...
app.post('/api/prompts', async (req, res) => {
  ...
  if (collectionId) {
    const cnt = await db.get<{ cnt: number }>('SELECT COUNT(1) as cnt FROM prompts WHERE collection_id = ?;', [collectionId])
...
app.post('/api/prompts/import', async (req, res) => {
  ...
  const stmt = await db.prepare(
    'INSERT INTO prompts (id, library, riskType, riskSubType, prompt, createdAt, collection_id) VALUES (?, ?, ?, ?, ?, ?, ?);',
  )
  try {
...
app.post('/api/evaluations', async (req, res) => {
  ...
  if (parsed.data.collectionId) {
    const row = await db.get<{ cnt: number }>('SELECT COUNT(1) as cnt FROM prompts WHERE collection_id = ?;', [parsed.data.collectionId])
    maxCount = row?.cnt ?? 0
  } else {
    const row = await db.get<{ cnt: number }>('SELECT COUNT(1) as cnt FROM prompts WHERE library = ?;', [parsed.data.standard])
    maxCount = row?.cnt ?? 0
  }
...
app.get('/api/evaluations/:id/report', async (req, res) => {
  const id = req.params.id
  const evaluation = await db.get<any>(
...
app.get('/api/mcp-scans', async (req, res) => {
  ...
  let filtered = rows.map(r => {
    ...
    return {
      ...
      progress: { stage: r.progress_stage as string, percent: r.progress_percent as number },
      createdAt: r.created_at as number,
      error: r.error as string,
      scoreTotal: r.score_total as number,
      scoreRiskLevel: r.score_risk_level as string,
...
app.post('/api/code-audit', async (req, res) => {
  ...
  let modelConfig: { provider: 'ollama' | 'openai' | 'anthropic' | 'zhipu'; baseUrl: string; apiKey?: string; model?: string; systemPrompt?: string; timeoutMs?: number }
  if (data.baseUrl) {
    modelConfig = { provider: data.provider, baseUrl: data.baseUrl, apiKey: data.apiKey, model: data.model, timeoutMs: data.timeoutMs }
  } else {
    const settingsRow = await db.get<{ value: string }>('SELECT value FROM settings WHERE key = ?;', ['codeAudit'])
...
app.get('/api/code-audit/:id/items', async (req, res) => {
  ...
  const options: { severity?: string; confidence?: string; limit: number; offset: number } = {
    limit: pageSize,
    offset: (page - 1) * pageSize,
  }
  if (severity && severity !== 'all') options.severity = severity
  if (confidence && confidence !== 'all') options.confidence = confidence
...
function generateHtmlReport(data: {
  name: string; status: string; language: string; riskScore: number
  totalFiles: number; totalSlices: number; findingsCount: number
...
function generateMarkdownReport(data: {
  ...
  for (const item of data.items) {
    ...
    lines.push('')
    if (item.cwe_id) lines.push(`- **CWE**: ${item.cwe_id}`)
    ...
    if (item.poc_description || item.poc_code) {
      lines.push('**概念验证(PoC)**:')
      lines.push('')
      if (item.poc_description) lines.push(item.poc_description)
      if (item.poc_code) {
        lines.push('```')
...
app.post('/api/skills-audit', async (req, res) => {
  ...
  if (data.baseUrl && data.baseUrl !== 'http://localhost:11434') {
    ...
    const settingsRow = await db.get<{ value: string }>('SELECT value FROM settings WHERE key = ?;', ['skillsAudit'])
    const settingsValue = settingsRow ? JSON.parse(settingsRow.value) : null
    modelConfig = {
      provider: settingsValue?.provider ?? data.provider,
...
app.get('/api/skills-audit/:id/items', async (req, res) => {
  ...
  const options: { riskCategory?: string; severity?: string; limit: number; offset: number } = {
    limit: pageSize,
    offset: (page - 1) * pageSize,
  }
  if (riskCategory && riskCategory !== 'all') options.riskCategory = riskCategory
  if (severity && severity !== 'all') options.severity = severity
...
async function pingModel(input: { provider: 'ollama' | 'openai' | 'anthropic' | 'zhipu'; baseUrl: string; apiKey?: string; model?: string }) {
...
/** 生成评估报告Markdown格式 */
function generateEvaluationMd(evaluation: any, items: any[]): string {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (score != null) lines.push(`- 评分: ${score}`)
    lines.push('')
  }
  lines.push(`---`, `*报告生成时间: ${new Date().toLocaleString('zh-CN')}*`, `*听风*`)
  return lines.join('\n')

...
import type { SkillsAuditAgent, AgentContext, AgentFinding, RiskCategory } from './types.js'

// ===== 通用LLM审计调用 =====

async function llmAudit(
  ctx: AgentContext,
  agentName: string,
  ...
  // Skills描述
  const skillsDesc = ctx.skills.map(s =>
    `Skill: ${s.name}${s.description ? ` - ${s.description}` : ''}${s.triggers ? `\n  Triggers: ${s.triggers.join(', ')}` : ''}`
  ).join('\n')
...
请分析以上信息，识别${riskCategories.join('/')}相关的安全风险，以JSON数组格式返回。`
  ...
  try {
    ...
    return findings.map(f => ({
      riskCategory: (f.riskCategory as RiskCategory) ?? riskCategories[0],
...
export const CommandAgent: SkillsAuditAgent = {
  name: 'CommandAgent',
  riskCategories: ['dangerous_command', 'reverse_shell', 'command_injection'],
  async audit(ctx) {
    return llmAudit(ctx, 'CommandAgent', ['dangerous_command', 'reverse_shell', 'command_injection'],
...
export const InjectionAgent: SkillsAuditAgent = {
  name: 'InjectionAgent',
  riskCategories: ['dynamic_code_execution', 'prompt_injection'],
  async audit(ctx) {
    return llmAudit(ctx, 'InjectionAgent', ['dynamic_code_execution', 'prompt_injection'],
...
export const SupplyChainAgent: SkillsAuditAgent = {
  name: 'SupplyChainAgent',
  riskCategories: ['supply_chain_attack', 'bytecode_tampering'],
  async audit(ctx) {
    return llmAudit(ctx, 'SupplyChainAgent', ['supply_chain_attack', 'bytecode_tampering'],
...
export const SkillManifestAgent: SkillsAuditAgent = {
  name: 'SkillManifestAgent',
  riskCategories: ['trigger_hijacking', 'skill_md_mismatch'],
  async audit(ctx) {
    return llmAudit(ctx, 'SkillManifestAgent', ['trigger_hijacking', 'skill_md_mismatch'],
...
export const QualityAgent: SkillsAuditAgent = {
  name: 'QualityAgent',
  riskCategories: ['code_quality', 'resource_abuse', 'obfuscation'],
  async audit(ctx) {
    return llmAudit(ctx, 'QualityAgent', ['code_quality', 'resource_abuse', 'obfuscation'],
...
export const SteganographyAgent: SkillsAuditAgent = {
  name: 'SteganographyAgent',
  riskCategories: ['unicode_steganography'],
  async audit(ctx) {
    return llmAudit(ctx, 'SteganographyAgent', ['unicode_steganography'],
      '重点检查：\n1. Unicode隐写（零宽字符\\u200b-\\u200f、\\u202a-\\u202e、同形字攻击）\n2. 隐藏的控制字符（BOM、软连字符\\u00ad、不可见字符）\n3. 字符编码欺骗（看起来正常但包含隐藏字符的变量名或URL）')
  },
}

/** SocialEngAgent: 社会工程 */
export const SocialEngAgent: SkillsAuditAgent = {
  name: 'SocialEngAgent',
  riskCategories: ['social_engineering'],
  async audit(ctx) {
    return llmAudit(ctx, 'SocialEngAgent', ['social_engineering'],
      '重点检查：\n1. 社会工程攻击（伪装系统安全警告、诱导输入密码、账户锁定欺骗）\n2. Skill输出中包含操纵用户行为的措辞\n3. 要求用户执行不安全操作的指令')
  },
}

/** 所有Agent列表 */
export const ALL_AGENTS: SkillsAuditAgent[] = [
  CommandAgent,
  InjectionAgent,
  CredentialAgent,
  ExfilAgent,

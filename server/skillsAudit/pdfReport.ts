...
import type { AgentFinding, Severity, RiskCategory } from './types.js'

/** PDFKit 文档实例类型 */
type PDFDoc = InstanceType<typeof PDFDocument>

/** Skills 安全审计PDF报告数据接口 */
interface SkillsPdfReportData {
  name: string; originalFilename: string
  status: string; riskScore: number; riskLevel: string
  totalSkills: number; totalFiles: number; findingsCount: number
 ...
 */
export function generateSkillsPdfReport(data: SkillsPdfReportData, res: Response): void {
...
/** 绘制封面页 */
function drawCoverPage(doc: PDFDoc, data: SkillsPdfReportData, reg: string, aero: string, song: string, fang: string, tnr: string): void {
  // 封面主标题：二号 航天腾飞体 居中
...
/** 按Agent分组统计 */
function groupByAgent(findings: AgentFinding[]): Record<string, { count: number; categories: Set<string> }> {
  const agentMap: Record<string, { count: number; categories: Set<string> }> = {}
  for (const f of findings) {
    const agent = categoryToAgent(f.riskCategory)
    if (!agentMap[agent]) agentMap[agent] = { count: 0, categories: new Set() }
...
/** 风险类别 → Agent 映射 */
function categoryToAgent(cat: RiskCategory): string {
 ...
 * Agent名称用小三，覆盖用五号
 */
function drawAgentStatsPage(doc: PDFDoc, data: SkillsPdfReportData, reg: string, bold: string, aero: string, song: string, fang: string): void {
 ...
 * 一级标题：风险类别分布 — 三号居中
 */
function drawCategoryPage(doc: PDFDoc, data: SkillsPdfReportData, reg: string, bold: string, aero: string, fang: string): void {
  drawH1(doc, '风险类别分布', aero)

  const categories = Object.entries(data.riskCategoryCount)
    .sort(([, a], [, b]) => (b as number) - (a as number))
  const maxCnt = Math.max(...categories.map(([, v]) => v as number), 1)

  for (const [cat, cnt] of categories) {
    ensureSpace(doc, 36)
    const label = RISK_CATEGORY_LABELS[cat] || cat
    const barW = ((cnt as number) / maxCnt) * (PAGE.CONTENT_WIDTH - 160)
...
/** 按严重度分组 */
function groupBySeverity(items: AgentFinding[]): Record<string, AgentFinding[]> {
 ...
 */
function drawFindingPages(doc: PDFDoc, data: SkillsPdfReportData, reg: string, mono: string, aero: string, song: string, fang: string): void {
...
function drawFindingCard(
  doc: PDFDoc, item: AgentFinding, severity: Severity, index: number,
  ...
  // 元信息
  const meta: string[] = []
  if (item.riskCategory) meta.push(`风险类别: ${RISK_CATEGORY_LABELS[item.riskCategory] || item.riskCategory}`)
  if (item.filePath) meta.push(`文件: ${item.filePath}`)
  if (item.cweId) meta.push(`CWE: ${item.cweId}`)
  if (item.confidence) meta.push(`置信度: ${item.confidence}`)
  if (meta.length > 0) {
    ensureSpace(doc, 20)

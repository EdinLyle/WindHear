...
export function registerFonts(doc: PDFDoc): {
  reg: string; bold: string; mono: string; monoBold: string;
  aero: string; song: string; fang: string; tnr: string;
} {
...
// ========== 封面标题区（二号 航天腾飞体，居中） ==========
export function drawCoverTitle(doc: PDFDoc, title: string, subtitle: string): void {
...
// ========== 报告头部（旧版，保留兼容） ==========
export function drawReportHeader(doc: PDFDoc, opts: {
  title: string;
  subtitle?: string;
  moduleName: string;
  date: string;
...
// ========== 评分板 ==========
export function drawScoreBoard(doc: PDFDoc, opts: {
  score: number;
  scoreLabel: string;
...
// ========== 一级标题（三号，居中） ==========
export function drawH1(doc: PDFDoc, title: string, aeroFont: string): void {
  ensureSpace(doc, 40);
...
export function drawSectionTitle(doc: PDFDoc, title: string, boldFont: string): void {
  ensureSpace(doc, 30);
...
// ========== 代码块（代码片段内容五号字体） ==========
export function drawCodeBlock(doc: PDFDoc, opts: {
  label?: string;
  code: string;
  labelColor: string;
  x: number;
  ...
  if (!opts.code || !opts.code.trim()) return;
...
// ========== 严重等级标签（小三 航天腾飞体） ==========
export function drawSeverityTag(doc: PDFDoc, severity: string, aeroFont: string, x?: number, y?: number): { width: number; height: number } {
  const sev = severity.toLowerCase();
  const bgKey = `${sev}Bg` as string;
  const borderKey = `${sev}Border` as string;
  const textKey = `${sev}Text` as string;
...
export function drawLineTag(doc: PDFDoc, text: string, monoFont: string, x?: number, y?: number): void {
  if (!text) return;
  const tagX = x ?? doc.x;
  const tagY = y ?? doc.y;
...
export function drawTable(doc: PDFDoc, rows: Array<{ label: string; value: string }>, options: {
  regFont: string;
  aeroFont: string;
  fangFont: string;
  tnrFont: string;
...
export function drawSeverityBar(doc: PDFDoc, severityCount: Record<string, number>, findingsCount: number): void {
  ...
  for (const sev of severityOrder) {
    const count = severityCount[sev] || 0;
    if (count > 0) legendParts.push(`${SEVERITY_ZH[sev.toUpperCase()]}: ${count}`);
  }
  doc.font('SongTi').fontSize(FONT_SIZES.header).fillColor(LIGHT_COLORS.textSecondary)

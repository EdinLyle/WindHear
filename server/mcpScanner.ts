...
export async function runMcpScan(input: {
  ...
  zipBuffer: Buffer
  store: McpScanStore
  options?: { model?: string; apiKey?: string; baseUrl?: string; provider?: 'ollama' | 'openai' | 'anthropic' | 'zhipu'; systemPrompt?: string }
}) {
  ...
  try {
    ...
    const scanOptions: ScanOptions = { model: options?.model, apiKey: options?.apiKey, baseUrl: options?.baseUrl, provider: options?.provider }

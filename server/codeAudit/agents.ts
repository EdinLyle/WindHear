...
import type { CodeSlice, ParserResult, RawFinding, ValidatedFinding, AuditReport, PipelineContext } from './types.js'
...
export async function runParser(ctx: PipelineContext): Promise<ParserResult[]> {
...
async function parseBatch(slices: CodeSlice[], ctx: PipelineContext): Promise<ParserResult[]> {
  const slicesDescription = slices.map((s, idx) =>
    `--- 切片 ${idx} ---\n文件: ${s.filePath}:${s.lineStart}-${s.lineEnd}\n类型: ${s.sliceType}\n语言: ${s.language}\n\`\`\`\n${s.content}\n\`\`\``
  ).join('\n\n')

  const systemPrompt = buildSystemPrompt(
    '你是代码安全分析专家（Parser）。你的任务是对代码切片进行初步安全标记。',
    '对每个切片，判断其信任边界：safe（无安全隐患）/ suspicious（涉及外部输入但无明显漏洞）/ dangerous（涉及危险操作）。\n输出JSON数组：[{index, trustBoundary, sensitiveTags, reason}]',
    ctx.modelConfig.systemPrompt,
  )
...
只输出 JSON 数组，不要输出其他内容。`
  ...
  try {
    ...
    return slices.map((_, idx) => {
      ...
      return {
        sliceIndex: idx,
        trustBoundary: (['safe', 'suspicious', 'dangerous'].includes(String(match?.trustBoundary ?? '')) ? String(match?.trustBoundary ?? 'safe') : 'safe') as ParserResult['trustBoundary'],
...
function ruleBasedParser(slice: CodeSlice, index: number): ParserResult {
  const content = slice.content.toLowerCase()
  const tags: string[] = []

  // 规则匹配
  const rules: Array<{ pattern: RegExp; tags: string[]; boundary: 'dangerous' | 'suspicious' }> = [
...
export async function runHunter(ctx: PipelineContext): Promise<RawFinding[]> {
...
async function huntBatch(slices: CodeSlice[], indices: number[], ctx: PipelineContext): Promise<RawFinding[]> {
  const vulnKbSummary = ctx.vulnKbPatterns.slice(0, 10).map(v =>
    `${v.cweId}(${v.cweName}): ${v.severity}`
  ).join(', ')

  const slicesDescription = slices.map((s, idx) =>
    `--- 切片 ${idx} [文件: ${s.filePath}:${s.lineStart}-${s.lineEnd}] ---\n\`\`\`${s.language}\n${s.content}\n\`\`\``
  ).join('\n\n')
  ...
  const systemPrompt = buildSystemPrompt(
    '你是攻击者视角的安全专家（Hunter）。你的任务是在代码切片中挖掘安全漏洞。',
    '对发现的每个漏洞输出：sliceIndex, cweId, cweName, title, description, lineStart, lineEnd, vulnerableCode, severity, confidence, exploitability, dataFlow, fixSuggestion, pocDescription, pocCode, reproduceSteps, fixCode。\n输出JSON数组，无漏洞则输出 []。',
    ctx.modelConfig.systemPrompt,
  )
  ...
  try {
    ...
    for (const item of parsed) {
      ...
      findings.push({
        ...
        severity: ['critical', 'high', 'medium', 'low', 'info'].includes(String(row.severity)) ? String(row.severity) as RawFinding['severity'] : 'medium',
        confidence: ['high', 'medium', 'low'].includes(String(row.confidence)) ? String(row.confidence) as RawFinding['confidence'] : 'medium',
...
export async function runValidator(ctx: PipelineContext): Promise<ValidatedFinding[]> {
  if (ctx.rawFindings.length === 0) return []
...
async function validateBatch(findings: RawFinding[], ctx: PipelineContext): Promise<ValidatedFinding[]> {
  const findingsDescription = findings.map((f, idx) =>
  ...
  const systemPrompt = buildSystemPrompt(
    '你是防御者视角的安全验证专家（Validator）。你的任务是验证漏洞发现是否为真实漏洞，排除误报。',
    '对每个发现输出：index, validated(true|false), validationReason, adjustedConfidence, adjustedSeverity(可选), improvedFixSuggestion(可选), improvedFixCode(可选), pocVerified(true|false)。\n输出JSON数组。',
    ctx.modelConfig.systemPrompt,
  )
...
只输出 JSON 数组。`
  ...
  try {
    ...
    return findings.map((f, idx) => {
      ...
      return {
        ...f,
        confidence: ['high', 'medium', 'low'].includes(String(match?.adjustedConfidence ?? '')) ? String(match?.adjustedConfidence) as ValidatedFinding['confidence'] : f.confidence,
        severity: ['critical', 'high', 'medium', 'low', 'info'].includes(adjSeverity) ? adjSeverity as ValidatedFinding['severity'] : f.severity,
...
export async function runReporter(ctx: PipelineContext): Promise<AuditReport> {
  ...
  const findingsSummary = confirmedFindings.slice(0, 20).map(f =>
    `- [${f.severity.toUpperCase()}] ${f.cweId}: ${f.title} (${f.filePath}:${f.lineStart})`
  ).join('\n')
...
function calculateRiskScore(findings: ValidatedFinding[]): number {

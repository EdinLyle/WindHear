...
// 漏洞条目类型
interface VulnItem {
  title?: string; cwe_id?: string; file_path?: string
  line_start?: number; confidence?: string
  description?: string; vulnerable_code?: string
  poc_description?: string; poc_code?: string
  reproduce_steps?: string; fix_code?: string
  fix_description?: string; fix_suggestion?: string
  severity?: string; [key: string]: unknown
}

// 报告数据接口
interface PdfReportData {
  name: string; originalFilename: string; status: string; language: string; riskScore: number
  ...
  items: VulnItem[]; logs: Record<string, unknown>[]; createdAt: number; completedAt: number
}
 ...
 * 生成PDF报告并流式输出到HTTP响应
 */
export function generatePdfReport(data: PdfReportData, res: Response): void {
 ...
 * 按严重度分组
 */
function groupBySeverity(items: VulnItem[]): Record<string, VulnItem[]> {
...
function drawVulnerabilityPages(
  doc: PDFDoc, data: PdfReportData,
  regFont: string, monoFont: string, aeroFont: string, songFont: string, fangFont: string
): void {
...
function drawVulnCard(
  doc: PDFDoc, item: VulnItem, severity: string, index: number,
  regFont: string, monoFont: string, aeroFont: string, songFont: string, fangFont: string
): void {
  ...
  if (item.cwe_id || item.file_path || item.confidence) {
    ensureSpace(doc, 20)
    const meta: string[] = []
    if (item.cwe_id) meta.push(`CWE: ${item.cwe_id}`)
    if (item.file_path) meta.push(`文件: ${item.file_path}`)

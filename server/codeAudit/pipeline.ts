...
import type { CodeAuditStore } from '../codeAuditStore.js'
...
import type { ValidatedFinding } from './types.js'

// 漏洞去重：基于文件路径 + CWE ID + 行号范围重叠
function dedupFindings(findings: ValidatedFinding[]): ValidatedFinding[] {
...
export async function runCodeAuditPipeline(input: {
  ...
  try {
    ...
    let projectFiles: Array<{ path: string; language: string; size: number }>
    ...
    const vulnKbPatterns = vulnKbRows.map((r: Record<string, unknown>) => ({

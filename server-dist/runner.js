...
export function startEvaluation(db, evaluationId) {
    if (running.has(evaluationId))
        return;
...
async function runEvaluation(db, evaluationId) {
    const evaluation = await db.get('SELECT * FROM evaluations WHERE id = ?;', [evaluationId]);
    if (!evaluation)
        return;
    ...
    const prompts = await db.all(evaluation.collectionId
        ...
        : (evaluation.totalCount === -1
            ...
            : `SELECT id, prompt, riskType, riskSubType FROM prompts WHERE library = ? ORDER BY RANDOM() LIMIT ?;`), evaluation.collectionId
    ...
    try {
        for (const p of prompts) {
            ...
            if (passed)
                ...
                failCount += 1;
            await db.run(`INSERT INTO evaluation_items
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`, [
            ...
            await db.run('UPDATE evaluations SET passCount = ?, failCount = ?, passRate = ? WHERE id = ?;', [passCount, failCount, passRate, evaluationId]);
        }
        const finishedAt = Date.now();
        const total = passCount + failCount;
        const passRate = total === 0 ? 0 : passCount / total;
        await db.run('UPDATE evaluations SET status = ?, finishedAt = ?, passCount = ?, failCount = ?, passRate = ? WHERE id = ?;', ['completed', finishedAt, passCount, failCount, passRate, evaluationId]);
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
...
async function getEvaluatorSettings(db) {
...
async function scoreWithEvaluator(config, systemPrompt, input) {

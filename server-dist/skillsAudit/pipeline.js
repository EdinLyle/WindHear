...
export async function runSkillsAuditPipeline(input) {
    ...
    try {
        ...
        const agentPromises = ALL_AGENTS.map(async (agent, idx) => {
            try {
                const findings = await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Agent timeout')), 90_000)),
                ]);
        ...
        for (const finding of ctx.agentFindings) {
            const hasRegexMatch = ctx.regexMatches.some(m => m.filePath === finding.filePath && m.riskCategory === finding.riskCategory);
...
/** 解析Skills结构 */
async function parseSkills(projectFiles, extractDir) {
    const skills = [];
    let manifest = null;
    // 查找SKILL.MD文件
    const skillMdFiles = projectFiles.filter(f => f.path.toLowerCase().includes('skill.md') || f.path.toLowerCase().includes('skill.md'));
    ...
    for (const [dir, files] of dirMap) {
        ...
        if (codeFiles.length > 0 || name.toLowerCase().includes('skill')) {
            const entryFile = codeFiles.find(f => ['main.py', 'index.js', 'index.ts', 'app.py', 'handler.py', 'main.js'].some(e => f.path.endsWith(e)));
            skills.push({
                name,
...
function dedupFindings(findings) {
    ...
    for (const f of findings) {
        const dupIdx = result.findIndex(r => r.filePath === f.filePath &&
...
/** 生成Markdown报告 */
function generateMarkdownReport(ctx, riskScore, riskLevel) {
    const severityEmoji = {
    ...
    for (const f of sorted) {
        ...
        if (f.cweId)
            lines.push(`- **CWE**: ${f.cweId} - ${f.cweName ?? ''}`);
        lines.push(``);

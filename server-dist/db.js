...
async function migrate(db) {
  ...
  `);
    ...
    const promptColumns = await db.all('PRAGMA table_info(prompts);');
    ...
    const evalItemColumns = await db.all('PRAGMA table_info(evaluation_items);');
    ...
    const evalColumns = await db.all('PRAGMA table_info(evaluations);');
    ...
    const mcpColumns = await db.all('PRAGMA table_info(mcp_scans);');
    ...
    const mcpReportsColumns = await db.all('PRAGMA table_info(mcp_reports);');
  ...
  `);
    // 初始化 CWE Top 20 种子数据
    const kbCount = await db.get('SELECT COUNT(*) as cnt FROM vuln_kb;');
    ...
    const existingCollections = await db.get('SELECT COUNT(*) as cnt FROM prompt_collections;');
    if (existingCollections && existingCollections.cnt === 0) {
        // 对 tc260/general: 按 riskType 分组创建 collection
        for (const lib of ['tc260', 'general']) {
            const groups = await db.all('SELECT riskType, COUNT(1) as cnt FROM prompts WHERE library = ? GROUP BY riskType;', [lib]);
            for (const g of groups) {
                const collId = crypto.randomUUID();
                const now = Date.now();
                await db.run('INSERT INTO prompt_collections (id, name, library, description, prompt_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?);', [collId, g.riskType, lib, null, g.cnt, now, now]);
        ...
        const customCount = await db.get('SELECT COUNT(1) as cnt FROM prompts WHERE library = \'custom\';');
        if (customCount && customCount.cnt > 0) {
            const collId = crypto.randomUUID();
            const now = Date.now();
            await db.run('INSERT INTO prompt_collections (id, name, library, description, prompt_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?);', [collId, '默认自定义测试集', 'custom', null, customCount.cnt, now, now]);
            await db.run('UPDATE prompts SET collection_id = ? WHERE library = \'custom\' AND collection_id IS NULL;', [collId]);
        }
    }
    // ===== 时间戳迁移：秒级 → 毫秒级 =====
    const auditCount = await db.get('SELECT COUNT(*) as cnt FROM code_audits;');
    if (auditCount && auditCount.cnt > 0) {
        const sample = await db.get('SELECT created_at FROM code_audits LIMIT 1;');
    ...
    // ===== CodeAudit 增强字段迁移 =====
    const auditItemsColumns = await db.all('PRAGMA table_info(code_audit_items);');
    ...
    const auditColumns = await db.all('PRAGMA table_info(code_audits);');
  ...
  `);
    ...
    const evaluatorRow = await db.get("SELECT value FROM settings WHERE key = 'evaluator';");
    if (evaluatorRow) {
        try {
            const evVal = JSON.parse(evaluatorRow.value);
            // 迁移到 modelEval（完整复制 evaluator 配置）
            const modelEvalRow = await db.get("SELECT value FROM settings WHERE key = 'modelEval';");
            if (!modelEvalRow) {
    ...
    const evalTimeoutColumns = await db.all("PRAGMA table_info(evaluations);");
    ...
    const skillsKbCount = await db.get("SELECT COUNT(*) as cnt FROM vuln_kb WHERE cwe_id IN ('CWE-88','CWE-1423','CWE-1357','CWE-1422','CWE-1424','CWE-1007','CWE-1421','CWE-345','CWE-451');");

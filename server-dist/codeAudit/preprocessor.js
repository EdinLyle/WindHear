...
export async function preprocessZip(zipBuffer, extractDir) {
    await fs.mkdir(extractDir, { recursive: true });
    await unzipToDirectory(zipBuffer, extractDir);
    return scanProject(extractDir);
}
export async function preprocessGit(gitUrl, extractDir) {
    await fs.mkdir(extractDir, { recursive: true });
    ...
    await new Promise((resolve, reject) => {
        ...
        execFile('git', ['clone', '--depth', '1', gitUrl, extractDir], (err) => {
            ...
            if (err)
                reject(new Error(`Git 克隆失败: ${err.message}`));
            else
                resolve();
        });
    });
...
export async function readSourceFile(rootDir, filePath) {
